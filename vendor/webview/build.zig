...
pub fn build(b: *std.Build) void {
    ...
    switch (target.query.os_tag orelse @import("builtin").os.tag) {
        ...
        .macos => {
            staticLib.addCSourceFile(.{ .file = webview.path("core/src/webview.cc"), .flags = &.{"-std=c++17", "-Wno-elaborated-enum-base"} });
        ...
        .freebsd => {
            ...
            staticLib.addCSourceFile(.{ .file = webview.path("core/src/webview.cc"), .flags = &.{ "-std=c++11", "-D__time64_t=long" } });
    ...
    switch (target.query.os_tag orelse @import("builtin").os.tag) {
        ...
        .macos => {
            sharedLib.addCSourceFile(.{ .file = webview.path("core/src/webview.cc"), .flags = &.{"-std=c++17", "-Wno-elaborated-enum-base"} });
        ...
        .freebsd => {
            ...
            sharedLib.addCSourceFile(.{ .file = webview.path("core/src/webview.cc"), .flags = &.{ "-std=c++11", "-D__time64_t=long" } });
