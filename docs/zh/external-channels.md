...
这页专门说明 `channels.external` 运行时，以及如何在不把渠道专用代码并入 core 的前提下为 krustyklaw 增加新渠道。
...
- 不把渠道专用 SDK、sidecar、bridge 逻辑塞进 krustyklaw core
...
- process model：由 krustyklaw 启动的子进程
...
- `runtime_name`
  运行时 channel 名称，会参与 routing、bindings、session key、daemon dispatch，以及 `krustyklaw channel start <runtime_name>`。
- `transport.command`
  插件进程的可执行路径或命令名。
- `transport.args`
  可选参数数组。
...
配置里的 `transport.timeout_ms` 并不意味着所有 control path 都会真的等这么久。KrustyKlaw 会对 health 和 supervision 敏感的请求施加更短的内部上限。
...
host/plugin 边界虽然很窄，但插件本质上仍然是以 krustyklaw 用户权限运行的本地进程。
