...
4. 用 `krustyklaw gateway` 启动 runtime，再用 `krustyklaw channel status`
   确认 DingTalk 显示为 healthy。
5. 如果怀疑凭据问题，可执行
   `krustyklaw --probe-channel-health --channel dingtalk --account <id>`
   验证 token 获取链路。
...
- 群聊回复目标过期后，krustyklaw 可以利用缓存的 conversation id 回退到
