# Integration Analysis: krustyklaw + nullboiler + nulltickets

## Executive Summary

This document analyzes the integration gaps between three krustyklaw ecosystem projects to enable end-to-end multi-agent workflows with Telegram topic routing.
...
```
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────┐
│   nulltickets   │────▶│   nullboiler     │────▶│    krustyklaw     │
...
| **krustyklaw** | AI agent runtime, tool execution, Telegram integration | `src/agent/root.zig`, `src/gateway.zig`, `src/session.zig` |

## Current Capabilities

### ✅ What Already Works

1. **Telegram Topic Delivery** (`krustyklaw/src/channels/telegram.zig:146-156`)
   - Supports `chatid#topic:threadid` format for forum topics
   - `MessageTool` can send to any channel/topic combination

2. **Agent Routing per Topic** (`krustyklaw/src/agent_bindings_config.zig`)
...
5. **krustyklaw Tool System** (`krustyklaw/src/tools/root.zig`)
...
6. **Agent Profile System** (`krustyklaw/src/config.zig`)
...
**Problem:** krustyklaw's gateway is inbound-only. No endpoint exists for external services (like nullboiler) to trigger outbound messages to specific Telegram topics.
...
**Required:**
- New `POST /api/send` endpoint in `krustyklaw/src/gateway.zig`
...
**Problem:** nullboiler can fire callbacks when steps complete, but krustyklaw has no endpoint to receive them.
...
- krustyklaw has no `/api/callback` endpoint

**Required:**
- New `POST /api/callback` endpoint in `krustyklaw/src/gateway.zig`
...
**Problem:** nullboiler dispatches steps to workers via HTTP POST, but krustyklaw has no endpoint that accepts nullboiler's step format.
...
- krustyklaw's `/webhook` is designed for Telegram, not generic worker tasks

**Required:**
- New `POST /api/worker` endpoint in `krustyklaw/src/gateway.zig`
...
**Problem:** krustyklaw cannot automatically register itself as a nullboiler worker on startup.

**Current State:**
- nullboiler has `POST /workers` endpoint (`nullboiler/src/api.zig:135`)
- krustyklaw has no startup registration logic

**Required:**
- Config additions: `nullboiler.url`, `nullboiler.worker_id`, `nullboiler.tags`, `nullboiler.capacity`
- Startup hook in `krustyklaw/src/daemon.zig` or new `krustyklaw/src/boiler_client.zig`
...
{
  "id": "krustyklaw-coder-1",
  "url": "http://krustyklaw:8080/api/worker",
  "protocol": "webhook",
  "tags": ["coder", "bash"],
  "max_concurrent": 3
}
...
- krustyklaw `http_request` tool can make arbitrary calls
- No high-level workflow abstraction

**Required:**
- New `create_workflow` tool in `krustyklaw/src/tools/workflow.zig`
- Wrap nullboiler `POST /runs` API
- Auto-set callback URL to krustyklaw's gateway
...
**Problem:** When nullboiler sends a callback, krustyklaw needs to know which Telegram topic to route the result to.
...
Without these, nullboiler cannot communicate with krustyklaw at all.
...
4. **Topic-Aware Routing**
   ...
   - Files: `krustyklaw/src/gateway.zig`, `nullboiler` run creation
...
5. **Self-Registration**
   ...
   - Health check pings
   - File: `krustyklaw/src/daemon.zig`
...
6. **Workflow Tool**
   ...
   - Schema validation
   - File: `krustyklaw/src/tools/workflow.zig`
...
### krustyklaw Gateway API Design

```zig
// New endpoints to add in krustyklaw/src/gateway.zig
...
pub fn fireCallbacks(
    ...
    // Callback POST to krustyklaw /api/callback
...
1. krustyklaw receives via Telegram webhook
...
5. Dispatches "code" step to "coder" worker (krustyklaw instance)
   → POST to krustyklaw /api/worker
...
9. Step completes
10. Fires callback
    → POST to krustyklaw /api/callback
...
```json
// krustyklaw config.json additions
{
  "nullboiler": {
    "url": "http://nullboiler:8080",
    "worker_id": "krustyklaw-main",
...
|---------|------|---------|
| krustyklaw | `src/gateway.zig` | Add 3 new endpoints: /api/worker, /api/send, /api/callback |
| krustyklaw | `src/config.zig` | Add nullboiler config struct |
| krustyklaw | `src/daemon.zig` | Add self-registration startup hook |
| krustyklaw | `src/tools/workflow.zig` | New tool for create_workflow |
| krustyklaw | `src/tools/root.zig` | Register workflow tool |
