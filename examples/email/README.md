...
This directory documents the recommended email integration shape for krustyklaw:
...
mailbox-specific workflows outside krustyklaw core while still using the generic
...
- generic operator workflow: `krustyklaw channel start <runtime_name>`
    ...
    "external": {
      "accounts": {
        "mailbox": {
          ...
          "transport": {
            "command": "/absolute/path/to/krustyklaw-channel-imap-connector/krustyklaw-plugin-imap-connector",
            "timeout_ms": 10000
          },
          "config": {
            "imap": {
...
`runtime.state_dir`. Relative paths resolve against the krustyklaw process cwd.

## Operator Flow

1. Clone or install `krustyklaw-channel-imap-connector`.
2. Configure IMAP/SMTP credentials in `channels.external.accounts.<id>.config`.
3. Start the external channel with `krustyklaw channel start imap_connector`.
...
- keeps email-specific dependencies out of the krustyklaw binary
- avoids growing `channel_loop.zig` with mailbox-specific polling code
- lets the email integration evolve independently from krustyklaw core releases
