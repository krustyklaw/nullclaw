//! Self-update command for krustyklaw.
//!
...
pub fn run(allocator: std.mem.Allocator, opts: Options) !void {
    ...
    std.debug.print("Restart krustyklaw to use the new version.\n", .{});
}
...
pub fn detectInstallMethod() !InstallMethod {
    ...
    // Check for docker
    if (std.mem.eql(u8, exe_path, "/krustyklaw")) {
        return .docker;
    }
...
fn printPackageManagerUpdate(method: InstallMethod) !void {
    ...
    const cmd = switch (method) {
        .nix => "nix-channel --update && nix-env -iA nixpkgs.krustyklaw",
        .homebrew => "brew upgrade krustyklaw",
        .docker => "docker pull ghcr.io/krustyklaw/krustyklaw:latest",
        else => unreachable,
    };
...
pub fn getLatestRelease(allocator: std.mem.Allocator) !ReleaseInfo {
    const url = "https://api.github.com/repos/krustyklaw/krustyklaw/releases/latest";
...
pub const PlatformTarget = enum {
    ...
    pub fn assetName(self: PlatformTarget) []const u8 {
        return switch (self) {
            .linux_x86_64 => "krustyklaw-linux-x86_64.bin",
            .linux_aarch64 => "krustyklaw-linux-aarch64.bin",
            .macos_aarch64 => "krustyklaw-macos-aarch64.bin",
            .macos_x86_64 => "krustyklaw-macos-x86_64.bin",
            .windows_x86_64 => "krustyklaw-windows-x86_64.exe",
        };
    }
};
...
fn findAssetUrl(allocator: std.mem.Allocator, asset_name: []const u8) ?[]const u8 {
    ...
    const base_url = "https://github.com/krustyklaw/krustyklaw/releases/latest/download/";
...
test "PlatformTarget.assetName covers all release assets" {
    try std.testing.expectEqualStrings("krustyklaw-linux-x86_64.bin", PlatformTarget.linux_x86_64.assetName());
    try std.testing.expectEqualStrings("krustyklaw-linux-aarch64.bin", PlatformTarget.linux_aarch64.assetName());
    try std.testing.expectEqualStrings("krustyklaw-macos-aarch64.bin", PlatformTarget.macos_aarch64.assetName());
    try std.testing.expectEqualStrings("krustyklaw-macos-x86_64.bin", PlatformTarget.macos_x86_64.assetName());
    try std.testing.expectEqualStrings("krustyklaw-windows-x86_64.exe", PlatformTarget.windows_x86_64.assetName());
}
